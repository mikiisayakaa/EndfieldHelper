...
def recognize_template(
    ...
    Args:
        full_screen_image: Full screen image as PIL Image
        template_name: Template filename (e.g., 'home_use_assistance.png')
        min_matches: Minimum number of good feature matches required
    
    Returns:
        Dict with keys: {
            'confidence': float (0-100 representing percentage),
            'x': int (center x),
            'y': int (center y),
            'x1': int (left),
            'y1': int (top),
    ...
    """
    template_path = SCREENSHOT_DIR / template_name
    ...
    print(f"Template recognized: {template_name}, Confidence: {confidence_percent:.1f}%, Center: ({center_x}, {center_y})")
