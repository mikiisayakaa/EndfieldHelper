...
自动战斗清体力（自动战斗不好做，不同队伍轴和练度不一样，很难打定番，暂时不考虑）、各种活动的自动化（非每日日常，开发收益小，没必要）

## 使用前提醒！！

由于助手仅在必要时使用图像识别，大部分操作都来自个人录制的操作脚本，直接使用脚本可能遇到：1.点击位置不对（屏幕分辨率不是2560 * 1600可能遇到），2.时间轴乱轴（电脑配置不同，有些过场加载时间比预设长），3.按键错误（使用了非默认的终末地键盘映射）。所以如果出错情况不多，建议可以用编辑功能修改一下具体配置或者自己重录一段，如果出错情况很多修改麻烦，只能抱歉找一下其他脚本了
